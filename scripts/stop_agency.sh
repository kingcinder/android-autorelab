...
def matches(proc: psutil.Process) -> bool:
    try:
        ...
        return False
    return (
        ("run_router.py" in cmdline and "--workflow agency" in cmdline)
        or ("workflow_service.py" in cmdline and "agency" in cmdline)
...
for proc in psutil.process_iter(["cmdline"]):
    try:
        ...
        continue
    if ("run_router.py" in cmdline and "--workflow agency" in cmdline) or (
        "workflow_service.py" in cmdline and "agency" in cmdline
    ) or ("llama-server" in cmdline and f"--port {port}" in cmdline):
        raise SystemExit(1)
