...
router_gone() {
  ! pgrep -f "run_router.py.*workflow legion" >/dev/null 2>&1 &&
...
pkill -f "run_router.py.*workflow legion" >/dev/null 2>&1 || true
pkill -f "workflow_service.py legion" >/dev/null 2>&1 || true
...
if ! wait_for_shutdown 15; then
  pkill -9 -f "run_router.py.*workflow legion" >/dev/null 2>&1 || true
