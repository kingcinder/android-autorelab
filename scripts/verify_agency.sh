...
assert_service_state() {
  ...
  if [ "$state" != "$expected" ]; then
    echo "[FAIL] expected $unit to be $expected, got ${state:-unknown}" >&2
    exit 1
  fi
}
...
grep -q '^ExecStartPre=.*scripts/stop_legion.sh' "$ROOT_DIR/services/agency.service"

"$ROOT_DIR/scripts/start_legion.sh" >/dev/null
wait_for_router http://127.0.0.1:18082/models 90
! "$PYTHON_BIN" "$ROOT_DIR/scripts/run_router.py" --repo-root "$ROOT_DIR" --workflow agency >/dev/null 2>&1
"$ROOT_DIR/scripts/start_agency.sh" >/dev/null
wait_for_router http://127.0.0.1:18081/models 90
assert_pid_matches agency
assert_lock_workflow agency
assert_port_listening "$PORT"

"$PYTHON_BIN" "$ROOT_DIR/scripts/workflow_verify.py" --repo-root "$ROOT_DIR" --workflow agency
printf '[PASS] Agency verification complete\n'
